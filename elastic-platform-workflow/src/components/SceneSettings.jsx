...
import {
  ...
  faUsers, faSliders, faPlus, faTrash, faDownload, faUpload, faLink
...
const DEMO_URL_KEY = 'presentation-demo-url'

export function useDemoUrl() {
  const [demoUrl, setDemoUrl] = useState(() => localStorage.getItem(DEMO_URL_KEY) || '')

  const updateDemoUrl = (url) => {
    const trimmed = url.trim()
    setDemoUrl(trimmed)
    if (trimmed) {
      localStorage.setItem(DEMO_URL_KEY, trimmed)
    } else {
      localStorage.removeItem(DEMO_URL_KEY)
    }
  }

  return { demoUrl: demoUrl.trim() || null, updateDemoUrl }
}

function DemoUrlInput({ demoUrl, onUpdateDemoUrl, isDark }) {
  const [isEditing, setIsEditing] = useState(false)
  const [value, setValue] = useState(demoUrl || '')
  ...
  // Keep local value in sync if external demoUrl changes
  useEffect(() => {
    if (!isEditing) setValue(demoUrl || '')
  }, [demoUrl, isEditing])

  const handleSubmit = () => {
    onUpdateDemoUrl(value)
    setIsEditing(false)
  }
  ...
  return isEditing ? (
    <div className="flex items-center gap-2">
      <input
        ...
        onKeyDown={(e) => {
          ...
          if (e.key === 'Escape') { setValue(demoUrl || ''); setIsEditing(false) }
        }}
        onBlur={handleSubmit}
        placeholder="https://elastic.navattic.com/..."
        className={`flex-1 px-3 py-1.5 text-xs rounded-lg border ${
          isDark
            ? 'bg-white/10 border-white/20 text-white placeholder-white/30'
            : 'bg-white border-elastic-dev-blue/20 text-elastic-dev-blue placeholder-elastic-dev-blue/30'
        }`}
      />
      <button
        onClick={handleSubmit}
        className="w-7 h-7 rounded-lg flex items-center justify-center bg-elastic-teal text-elastic-dev-blue flex-shrink-0"
      >
    ...
    <button
      ...
      className={`w-full flex items-center gap-2 text-xs px-3 py-2 rounded-lg text-left transition-all ${
        demoUrl
          ? isDark
            ? 'bg-elastic-teal/10 border border-elastic-teal/20 text-elastic-teal hover:bg-elastic-teal/20'
            : 'bg-elastic-blue/10 border border-elastic-blue/20 text-elastic-blue hover:bg-elastic-blue/20'
          : isDark
            ? 'bg-white/5 border border-white/10 text-white/40 hover:bg-white/10 hover:text-white/60'
            : 'bg-elastic-dev-blue/5 border border-elastic-dev-blue/10 text-elastic-dev-blue/40 hover:bg-elastic-dev-blue/10'
      }`}
      title="Click to set demo URL"
    >
      <FontAwesomeIcon icon={faLink} className="flex-shrink-0" />
      <span className="truncate">{demoUrl || 'Click to add Navattic demo URL…'}</span>
    </button>
  )
}
              ...
              <div className="flex-1 overflow-y-auto p-4">
                {activeTab === 'scenes' ? (
                  <>
                    ...
                    {/* Click-Through Demo URL */}
                    <div className={`mt-4 p-4 rounded-xl ${isDark ? 'bg-white/[0.03]' : 'bg-elastic-dev-blue/[0.02]'}`}>
                      <div className="flex items-center gap-2 mb-1">
                        <FontAwesomeIcon icon={faLink} className={`text-xs ${isDark ? 'text-white/40' : 'text-elastic-dev-blue/40'}`} />
                        <h3 className={`text-sm font-semibold ${isDark ? 'text-white/70' : 'text-elastic-dev-blue/70'}`}>
                          Click-Through Demo URL
                        </h3>
                      </div>
                      <p className={`text-xs mb-3 ${isDark ? 'text-white/30' : 'text-elastic-dev-blue/30'}`}>
                        Shown as "Launch Demo" in Next Steps. Leave blank to disable the button.
                      </p>
                      <DemoUrlInput demoUrl={demoUrl} onUpdateDemoUrl={onUpdateDemoUrl} isDark={isDark} />
                    </div>
                  </>
                ) : (
