...
import {
  faServer, faPlug, faCloudArrowUp, faMagnifyingGlassChart,
...
const steps = [
  {
    ...
    color: '#FF957D',
    label: 'Your Cluster',
    sublabel: 'ECE · ECK · Self-Managed',
    detail: 'Your Elasticsearch cluster running on-premise, in your own cloud, or on Kubernetes. No migration required.',
  },
  {
    id: 'agent',
    icon: faPlug,
    color: '#0B64DD',
    label: 'Elastic Agent',
    sublabel: 'Installed alongside your cluster',
    detail: 'A lightweight agent installed on your infrastructure. It connects to your Elasticsearch cluster and reads metrics using a dedicated API key with minimal privileges.',
  },
  {
    id: 'cloudconnect',
    icon: faCloudArrowUp,
    color: '#48EFCF',
    label: 'Cloud Connect API',
    sublabel: 'Registers & routes metrics',
    detail: 'The agent registers your cluster with Elastic Cloud via the Cloud Connect API, selecting your preferred CSP region. Only metrics travel to Elastic Cloud — your data stays put.',
  },
  {
    id: 'autoops',
    ...
    color: '#F04E98',
    label: 'AutoOps',
    sublabel: 'Elastic Cloud · Your CSP Region',
    detail: 'AutoOps receives the metrics stream, runs continuous analysis across hundreds of signals, detects issues, identifies root causes, and surfaces actionable recommendations in real time.',
  },
]
        ...
        <FontAwesomeIcon
          ...
          className={`absolute right-0 top-1/2 -translate-y-1/2 text-xs ${
            active
              ...
              : isDark ? 'text-white/20' : 'text-elastic-dev-blue/15'
          }`}
      ...
      <div className="max-w-6xl mx-auto w-full">
        ...
        >
          ...
          <p className={`text-paragraph text-lg mt-4 max-w-2xl mx-auto ${isDark ? 'text-elastic-light-grey' : 'text-elastic-ink'}`}>
            Three components. One data flow. Click a step or press play to walk through the architecture.
          </p>
        </motion.div>
          ...
          {steps.map((step, index) => (
            <div key={step.id} className="flex items-center flex-1">
                ...
                className={`flex flex-col items-center gap-2 p-4 rounded-2xl border-2 transition-all flex-shrink-0 w-36 ${
                  activeStep >= index || selectedStep === index
                    ? isDark
                      ...
                      : 'border-opacity-80 bg-white'
                    : isDark
                      ? 'border-white/10 bg-white/[0.02]'
                      : 'border-elastic-dev-blue/10 bg-white/40'
                }`}
                style={{
                  borderColor: activeStep >= index || selectedStep === index
                    ? step.color
                    : undefined,
                  boxShadow: activeStep === index || selectedStep === index
              ...
              {index < steps.length - 1 && (
                <FlowArrow active={activeStep > index} isDark={isDark} />
              )}
            </div>
          ))}
        </motion.div>
          ...
          {displayStep >= 0 && (
            <motion.div
              ...
              className={`p-6 rounded-xl border mb-6 ${
                isDark
                  ...
                  : 'bg-white border-elastic-dev-blue/10'
              }`}
            ...
            >
              <div className="flex items-start gap-4">
                ...
                >
                  <FontAwesomeIcon
                    icon={steps[displayStep].icon}
                    style={{ color: steps[displayStep].color }}
                    className="text-lg"
                  />
                </div>
                <div>
          ...
          {displayStep < 0 && (
            ...
            >
              <p className={`text-sm ${isDark ? 'text-white/30' : 'text-elastic-dev-blue/30'}`}>
                Press play to animate the flow, or click any component above to learn more.
              </p>
            </motion.div>
          )}
        </AnimatePresence>
        ...
        >
          <p className={`text-sm font-medium ${isDark ? 'text-elastic-teal' : 'text-elastic-blue'}`}>
            Your cluster data never leaves your environment — only metrics flow to AutoOps in your chosen CSP region.
          </p>
        </motion.div>
      </div>
    </div>
